export default function Rating({ rating, maxRating = 5, showValue = false, size = 'md' }) {
  const sizeClasses = {
    sm: 'w-3 h-3',
    md: 'w-4 h-4',
    lg: 'w-5 h-5'
  ...
  for (let i = 1; i <= maxRating; i++) {
    ...
    stars.push(
      <div key={i} className="relative inline-block">
        <svg className={`${sizeClasses[size]} text-neutral-200`} fill="currentColor" viewBox="0 0 20 20">
